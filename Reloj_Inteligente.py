...
COLOR_FONDO = TEMAS[tema_actual]["fondo"]
COLOR_TEXTO = TEMAS[tema_actual]["texto"]
COLOR_BOTON = TEMAS[tema_actual]["boton"]
COLOR_DIA_ACTUAL = TEMAS[tema_actual]["actual"]
...
def guardar_alarma():
    if alarma_activa:
        with open(ARCHIVO_ALARMAS, "w") as f:
            json.dump({"alarma": alarma_activa}, f)

def cargar_alarma():
    global alarma_activa
    if os.path.exists(ARCHIVO_ALARMAS):
        with open(ARCHIVO_ALARMAS, "r") as f:
            data = json.load(f)
            alarma_activa = data.get("alarma")

# ======================== CAMBIAR TEMA ==============================
def cambiar_tema():
    global tema_actual, COLOR_FONDO, COLOR_TEXTO, COLOR_BOTON, COLOR_DIA_ACTUAL
    tema_actual = (tema_actual + 1) % len(TEMAS)
    tema = TEMAS[tema_actual]
    COLOR_FONDO = tema["fondo"]
    COLOR_TEXTO = tema["texto"]
    COLOR_BOTON = tema["boton"]
    COLOR_DIA_ACTUAL = tema["actual"]
    root.configure(bg=COLOR_FONDO)
    etiqueta_hora.configure(bg=COLOR_FONDO, fg=COLOR_TEXTO)
    etiqueta_fecha.configure(bg=COLOR_FONDO, fg=COLOR_TEXTO)
...
def sonar_alarma():
    if platform.system() == "Windows":
        ...
        print("\a")  # beep en consola Linux/Mac
...
# Menú cambiar color
barra_menu.add_command(label="Cambiar color", command=cambiar_tema)
# Menú Salir
